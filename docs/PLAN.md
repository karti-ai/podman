...
- `GIT_REPORT` exists in shared types and agent handling, but there is no
  finished per-laptop git sidecar script.
...
5. **Scheduled git truth**
   - Add the local git sidecar or equivalent `GIT_REPORT` sender.
   - Report dirty files, branch, latest commit, and unpushed count about every
     minute.
   - Use git truth when vision is unsure about unpushed state.
...
- [ ] Local git report supplies dirty/unpushed truth on a schedule.
